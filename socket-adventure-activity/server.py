...
class Server(object):
    ...
    * self.room: one of 0, 1, 2, 3. This signifies which "room" the client is in,
    ...
    def room_description(self, room_number):
        ...
        rooms = {
            0: "This room has green wallpaper",
            1: "This room has red wallpaper",
            2: "This room has blue wallpaper",
            3: "This room has yellow wallpaper",
            }

        return str(rooms[room_number])
    ...
    def move(self, argument):
        ...
        * "west" 1
        ...
        self.output_buffer = room_description(self.room)
    ...
    def route(self):
        ...
        try:
            if list(action.split(' '))[0] == 'say':
                self.say(' '.join(list(action.split(' ')[1:])))
            if list(action.split(' '))[0] == 'move':
                self.move(' '.join(list(action.split(' ')[1])))
            if action == 'quit':
                self.quit(None)

        except Exception as e:
            self.say("I'm afraid I don't understand that.")
            print(e)
            pass
        '''
        # prof solution with interesting routing dict
    ...
    def push_output(self):
        ...
        # self.client_connection    # this is a socket
        # self.output_buffer    # this is a string
        # message = b"OK! " + self.output_buffer + b"\n"
